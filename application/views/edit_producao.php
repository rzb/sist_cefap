...
$(document).ready(function() {
	...
	$('#txtSaldoHoras').blur(function(){
	
		var intRegex = /^\-{0,1}\d{1,5}$/;
		var valor = $(this).val();
		if(!intRegex.test(valor))
		{
			alert('Digite apenas números, permitindo sinal de negativo no início.');
			$(this).val('');
			$(this).focus();
		}
	});	
	...
	$('#calcular_saldo').click(function(){
		var prev = $('#txtCHprevista').val();
		var cump = $('#txtCHcumprida').val();
		
		var saldo = cump - prev;
		saldo.toString();
		$('#txtSaldoHoras').val(saldo);
	});
});
...
if(true)
{
	...
	echo '<input type="text" size="5" name="txtSaldoHoras" id="txtSaldoHoras" class="saldo_horas" value="' . $saldo_horas . '"  />';
	echo " <span id='res_saldo'>" . $resultado_saldo_horas . "</span>";
	echo " <span id='calcular_saldo_span'><a href='' id='calcular_saldo' onclick='return false;'>calcular a partir dos valores acima</a></span>";
	echo "</div>";
	...
	echo "<br/><p><strong>Complementos dos plantões:</strong></p>";
	...
	echo "<br/><p><strong>Nível do assistente:</strong></p>";
	...
	echo '<input type="text" size="5" name="txtDesconto" id="txtDesconto" class="" value="' . $desconto . '"  />';
	echo "</div>";
	
	echo "<div class='field_holder'>";
	echo "<br/><p><strong>Anotações adicionais:</strong></p>";
	echo "<textarea name='txtObs' id='txtObs' rows='6' cols='150'>" . $obs . "</textarea>";
	echo "</div>";
